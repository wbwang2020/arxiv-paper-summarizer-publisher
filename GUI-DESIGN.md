...
- 启用飞书发布（开关）
- 应用ID输入（app_id）
- 应用密钥输入（app_secret，密码框）
- 文件夹Token输入（可选）
- 发送通知开关
- 通知用户ID列表（支持多用户，可添加/删除）
...
|  | 应用密钥:   [********************]            ||
|  |                                              ||
|  | 文件夹Token: [__________________________]     ||
|  |              （可选，留空则创建在根目录）       ||
|  |                                              ||
|  | 发送通知:   [x]                              ||
|  |                                              ||
...
- 飞书配置支持测试连接功能，验证app_id和app_secret是否有效
